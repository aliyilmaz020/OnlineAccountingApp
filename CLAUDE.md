...
This is a monorepo: the .NET backend lives in `api/` (contains `OnlineAccountingApp.slnx`); a `ui/` frontend folder is planned alongside it. Run all `dotnet` commands from `api/`, e.g. `cd api` first, or pass the solution/project path explicitly.
...
This project has a knowledge graph at `api/graphify-out/` (indexes the `api/` backend) with god nodes, community structure, and cross-file relationships.

Rules:
- For codebase questions, first run `graphify query "<question>"` when `api/graphify-out/graph.json` exists. Use `graphify path "<A>" "<B>"` for relationships and `graphify explain "<concept>"` for focused concepts. These return a scoped subgraph, usually much smaller than GRAPH_REPORT.md or raw grep output.
- If `api/graphify-out/wiki/index.md` exists, use it for broad navigation instead of raw source browsing.
- Read `api/graphify-out/GRAPH_REPORT.md` only for broad architecture review or when query/path/explain do not surface enough context.
- After modifying code, run `graphify update .` from within `api/` to keep the graph current (AST-only, no API cost).
